...
def keypress(event):
    ...
    def admin_ms():
        ...
        def new_opr():
            ...
            def updateopr():
                dbcon = sqlite3.connect("BUS MS")
                cursor = dbcon.execute("""UPDATE operator SET(name="{}",contact="{}",address="{}",email="{}") WHERE operator.id={};""".format(nameentry.get(), phoneentry.get(), addressentry.get(), emailentry.get(), oprentry.get()))
